class ContentController < ApplicationController
	...
	def new_user
		if User.find_by(jive_user_id: params[:jive_user_id]).blank?
			user = User.new(jive_user_id: params[:jive_user_id], employee_id: params[:employee_id], client_id: params[:client_id])
			if user.valid?
				user.save
			else
	...
	def get_structure
		...
		if user and !user.employee_id.blank?
			::NewRelic::Agent.increment_metric("Custom/SecondaryTopic/#{SecondaryTopic.find(params[:secondary]).name}")
